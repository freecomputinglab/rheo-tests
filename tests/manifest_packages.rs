...
/// E2e test: a package that itself imports another package contributes that
/// second package's assets, even though the project never imports the second
/// package directly (`scan_transitive_package_imports` in
/// `crates/core/src/packages/manifest.rs`).
#[test]
fn e2e_transitive_package_assets_are_collected() {
    let cache_dir = tempfile::tempdir().unwrap();
    let project_dir = tempfile::tempdir().unwrap();
    let project_path = project_dir.path();
    ...
    let leaf_pkg = FakePackage::new(&packages_root, "@e2ens/leafpkg:0.1.0");
    let leaf_pkg_dir = leaf_pkg.root().to_path_buf();
    std::fs::write(
        leaf_pkg_dir.join("typst.toml"),
        r#"
[package]
name = "leafpkg"
version = "0.1.0"
...
[tool.rheo.html]
css_stylesheet = "leaf.css"
js_scripts = "leaf.js"
"#,
    )
    .unwrap();
    std::fs::write(leaf_pkg_dir.join("leaf.css"), "body { color: green; }").unwrap();
    std::fs::write(leaf_pkg_dir.join("leaf.js"), "console.log('leaf');").unwrap();
    std::fs::write(leaf_pkg_dir.join("lib.typ"), "").unwrap();

    let root_pkg = FakePackage::new(&packages_root, "@e2ens/rootpkg:0.1.0");
    let root_pkg_dir = root_pkg.root().to_path_buf();
    std::fs::write(
        root_pkg_dir.join("typst.toml"),
        r#"
[package]
name = "rootpkg"
version = "0.1.0"
...
[tool.rheo.html]
css_stylesheet = "root.css"
js_scripts = "root.js"
"#,
    )
    .unwrap();
    std::fs::write(root_pkg_dir.join("root.css"), "body { color: red; }").unwrap();
    std::fs::write(root_pkg_dir.join("root.js"), "console.log('root');").unwrap();
    std::fs::write(
        root_pkg_dir.join("lib.typ"),
        "#import \"@e2ens/leafpkg:0.1.0\": *\n",
    )
    .unwrap();

    // The project imports only rootpkg — leafpkg is reached transitively,
    // through rootpkg's own lib.typ.
    std::fs::write(
        project_path.join("main.typ"),
        r#"#import "@e2ens/rootpkg:0.1.0": *
= Hello
Test document.
"#,
    )
    ...
    // leafpkg's assets land in the build even though the project never
    // imported it directly — only rootpkg pulled it in.
    assert!(
        build_dir.join("html/e2ens/leafpkg/leaf.css").exists(),
        "transitive package CSS not found at html/e2ens/leafpkg/leaf.css"
    );
    assert!(
        build_dir.join("html/e2ens/leafpkg/leaf.js").exists(),
        "transitive package JS not found at html/e2ens/leafpkg/leaf.js"
    );

    let html = std::fs::read_to_string(build_dir.join("html/main.html"))
        .expect("Failed to read HTML output");

    let root_needle = "e2ens/rootpkg/root.js";
    let leaf_needle = "e2ens/leafpkg/leaf.js";
    assert!(
        html.contains(root_needle) && html.contains(leaf_needle),
        "HTML should reference both root and leaf scripts:\n{html}"
    );

    // Order: the project's own specs come first, encounter order otherwise —
    // a transitively-reached package must not jump ahead of one the project
    // imported directly.
    let root_pos = html
        .find(root_needle)
        .expect("root script src should be present");
    let leaf_pos = html
        .find(leaf_needle)
        .expect("leaf script src should be present");
    assert!(
        root_pos < leaf_pos,
        "root package's <script> should appear before leaf's:\n{html}"
    );
}
